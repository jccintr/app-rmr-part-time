...
const SelectInput = ({label,placeholder,options,onChangeSelect}) => {
   ...
   const SelectItem = ({item}) => {
    ...
    return (
        <TouchableOpacity style={[styles.selectItem,{backgroundColor:item.id===selected?'#eee':'#fff'}]} onPress={()=>onChange(item)}>
              ...
              {item.id===selected&&<Entypo name="check" size={24} color={cores.azulEscuro}/>}
        </TouchableOpacity>
  ...
  return (
      ...
      <>
            ...
            <Text style={{color:cores.azul, marginBottom: 5,marginLeft:5,fontSize:14}}>{label}</Text>
